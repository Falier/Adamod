...
package net.mcreator.mymodpack.world.dimension;
...
import net.mcreator.mymodpack.item.AdamantDimensionItem;
import net.mcreator.mymodpack.block.RedWaterBlock;
import net.mcreator.mymodpack.block.FossilOreBlock;
...
public class AdamantDimensionDimension extends MyModpackElements.ModElement {
	@ObjectHolder("mymodpack:adamantdimension")
	public static final ModDimension dimension = null;
	@ObjectHolder("mymodpack:adamantdimension_portal")
	public static final CustomPortalBlock portal = null;
	public static DimensionType type = null;
	private static Biome[] dimensionBiomes;
	public AdamantDimensionDimension(MyModpackElements instance) {
		super(instance, 14);
	...
	public void registerDimension(RegistryEvent.Register<ModDimension> event) {
		event.getRegistry().register(new CustomModDimension().setRegistryName("adamantdimension"));
	}

	@SubscribeEvent
	public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("mymodpack:adamantdimension")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("mymodpack:adamantdimension"), dimension, null, false);
		}
		type = DimensionType.byName(new ResourceLocation("mymodpack:adamantdimension"));
	}
	...
	public void init(FMLCommonSetupEvent event) {
		dimensionBiomes = new Biome[]{ForgeRegistries.BIOMES.getValue(new ResourceLocation("swamp")),
				...
				ForgeRegistries.BIOMES.getValue(new ResourceLocation("mymodpack:miningbiome")),};
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new CustomPortalBlock());
		elements.items.add(() -> new AdamantDimensionItem().setRegistryName("adamantdimension"));
	}
	public static class CustomPortalBlock extends NetherPortalBlock {
		public CustomPortalBlock() {
			...
			setRegistryName("adamantdimension_portal");
		}
			...
			protected int getDistanceUntilEdge(BlockPos p_180120_1_, Direction p_180120_2_) {
				...
				for (i = 0; i < 22; ++i) {
					...
					if (!this.func_196900_a(this.world.getBlockState(blockpos))
							|| !(this.world.getBlockState(blockpos.down()).getBlock() == Blocks.NETHER_BRICKS.getDefaultState().getBlock())) {
						break;
					}
				}
				BlockPos framePos = p_180120_1_.offset(p_180120_2_, i);
				return (this.world.getBlockState(framePos).getBlock() == Blocks.NETHER_BRICKS.getDefaultState().getBlock()) ? i : 0;
			}
				...
				label56 : for (this.height = 0; this.height < 21; ++this.height) {
					for (int i = 0; i < this.width; ++i) {
						...
						if (i == 0) {
							...
							if (!(this.world.getBlockState(framePos).getBlock() == Blocks.NETHER_BRICKS.getDefaultState().getBlock())) {
								break label56;
							}
					...
					for (int i8 = 1; i8 < 3; ++i8) {
						for (int i9 = -1; i9 < 3; ++i9) {
							...
							this.world.setBlockState(blockpos$mutableblockpos,
									flag ? Blocks.NETHER_BRICKS.getDefaultState().getBlock().getDefaultState() : Blocks.AIR.getDefaultState());
						}
					}
				}
			}
			for (int k7 = -1; k7 < 3; ++k7) {
				for (int j8 = -1; j8 < 4; ++j8) {
					if (k7 == -1 || k7 == 2 || j8 == -1 || j8 == 3) {
						...
						this.world.setBlockState(blockpos$mutableblockpos, Blocks.NETHER_BRICKS.getDefaultState().getBlock().getDefaultState(), 3);
					}
				}
			}
		...
		public ChunkProviderModded(World world, BiomeProvider provider) {
			super(world, provider, new NetherGenSettings() {
				public BlockState getDefaultBlock() {
					return FossilOreBlock.block.getDefaultState();
				}

				public BlockState getDefaultFluid() {
					return RedWaterBlock.block.getDefaultState();
				}
			});
